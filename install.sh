...
if [ ! -t 0 ] && [ "$NON_INTERACTIVE" = false ]; then
    ...
    echo ""
    echo "Or run non-interactively (skips service configuration):"
    echo -e "  ${CYAN}bash install.sh --non-interactive${NC}"
    exit 1
fi

if [ "$NON_INTERACTIVE" = true ]; then
    info "Running in non-interactive mode. Service configuration will be skipped."
fi
...
if [ "$NEED_CONFIG" = true ] && [ "$NON_INTERACTIVE" = true ]; then
    info "Non-interactive mode: skipping Telegram configuration"
    # Create a placeholder config so the rest of the script doesn't break
    cat > "$CONFIG_FILE" << EOF
# Lobster Configuration
# Generated by installer on $(date) (non-interactive mode)
# Run the installer again without --non-interactive to configure Telegram
EOF
    NEED_CONFIG=false
    ...
    while true; do
        ...
        if [ -z "$BOT_TOKEN" ]; then
            info "Skipping Telegram bot token"
            break
    ...
    # Get user ID (only if token was provided)
    USER_ID=""
    if [ -n "$BOT_TOKEN" ]; then
        while true; do
            read -p "Telegram numeric user ID (press Enter to skip): " USER_ID
            if [ -z "$USER_ID" ]; then
                info "Skipping Telegram user ID"
                break
...
EOF
    ...
    if [ -n "$BOT_TOKEN" ] && [ -n "$USER_ID" ]; then
        ...
        info "Partial/empty Telegram config saved. Re-run installer or edit $CONFIG_FILE to complete setup."
    fi
fi
...
if [ "$NON_INTERACTIVE" = true ]; then
    info "Non-interactive mode: skipping GitHub integration setup"
    REPLY="n"
else
    echo ""
...
if [ "$NON_INTERACTIVE" = true ]; then
    REPLY="n"
    info "Non-interactive mode: skipping service start"
else
...
echo ""
if [ "$NON_INTERACTIVE" = true ]; then
    echo -e "${YELLOW}${BOLD}NOTE: Installed in non-interactive mode.${NC}"
    echo "Service configuration (Telegram, GitHub, Claude auth) was skipped."
    echo ""
    echo "To complete setup, run the installer again interactively:"
